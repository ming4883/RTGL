...
// must match NRCFrameParams in NrcCommon.glsl; 60 bytes
struct NRCFrameParamsGpu
{
    ...
    uint32_t uTrainBatchSize;
    float    uLearningRate;
    float    uEmaAlpha;
};

static_assert( sizeof( NRCFrameParamsGpu ) == 60 );
...
public:
    ...
    // max records the train records buffer can hold; runtime batch size is
    // clamped to it (raygen) and to the batch buffer capacity
    static constexpr uint32_t MAX_TRAIN_RECORDS =
        NRC_TRAIN_BATCH_COUNT * NRC_TRAIN_BATCH_SIZE;


    // zeroes eval/train counters and the indirect dispatch cmd at the
    // start of a frame, before the indirect raygen writes them
    void ZeroCounters( VkCommandBuffer cmd );
