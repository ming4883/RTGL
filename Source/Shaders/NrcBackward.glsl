...
void NNUpdateDW3( uint layer )
{
    ...
    if( gl_LocalInvocationID.x < WEIGHT_3_UV4_COUNT )
    {
        ...
        for( uint i = 0; i < FP16_PER_UV4 / 2; ++i )
        {
            ...
            // RTGL: plain atomicAdd (Device scope, no explicit semantics) - the extended
            // 4-arg form with gl_StorageSemanticsBuffer + gl_SemanticsRelaxed produced
            // OpAtomicFAddEXT with a storage-class semantics bit but no ordering bit,
            // which violates VUID-StandaloneSpirv-MemorySemantics-10871
            atomicAdd( uGradients[ g0 ], d_w[ i ].x );
            atomicAdd( uGradients[ g1 ], d_w[ i ].y );
        }
    }
}
...
void NNUpdateDW64( uint layer )
{
    ...
    for( uint u = 0; u < THREAD_WEIGHT_64_UV4_COUNT; ++u )
    {
        ...
        for( uint i = 0; i < ( FP16_PER_UV4 / 2 ); ++i )
        {
            uint g0 = min( kWeightFP16Base + ( i << 1u ),      NRC_WEIGHT_COUNT - 1 );
            uint g1 = min( kWeightFP16Base + ( i << 1u | 1u ), NRC_WEIGHT_COUNT - 1 );
            atomicAdd( uGradients[ g0 ], d_w[ u ][ i ].x );
            atomicAdd( uGradients[ g1 ], d_w[ u ][ i ].y );
        }
    }
}