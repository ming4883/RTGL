    ...
    , paramsAutoBuffer( aAllocator )
{
    ...
    if( !active )
    {
        debug::Warning( "NRC is disabled (no coopmat/atomic float support, or disabled in RTGL1.json)" );
        return;
    }
...
void NRCCache::CreateDescLayout()
{
    ...
    // optimizer state: SSBO - TrainPrepare writes it (advances Adam state),
    // Optimize reads it; a single descriptor type must serve both shaders
    make( 7, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER );  // optimizer state
...
void NRCCache::UpdateDescriptors()
{
    ...
    for( uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++ )
    {
        ...
        {
            makeWrite( b, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &infos[ b ] );
        }
                                  ...
                                  float                 emaAlpha )
{
    ...
    // clamp the runtime knobs to safe ranges: probability to [0,1], batch
    // size to the train records buffer capacity
    trainProbability = std::clamp( trainProbability, 0.0f, 1.0f );
    trainBatchSize   = std::clamp( trainBatchSize, 0u, MAX_TRAIN_RECORDS );
    ...
    NRCFrameParamsGpu params = {
        ...
        .uWidth              = width,
        .uHeight             = height,
        .uTrainProbability   = trainProbability,
        .uTrainBatchSize     = trainBatchSize,
        .uLearningRate       = learningRate,
        .uEmaAlpha           = emaAlpha,
    };
...
void NRCCache::ZeroCounters( VkCommandBuffer cmd )
{
    if( !active )
    {
        return;
    ...
    // zero counters + dispatch cmd (values in the beginning of each
    // buffer); must be recorded before the indirect raygen of this
    // frame, which atomically increments the counters
    vkCmdFillBuffer( cmd, evalCountBuffer, 0, sizeof( uint32_t ), 0 );
    vkCmdFillBuffer( cmd, batchTrainCountsBuffer, 0,
                     NRC_TRAIN_BATCH_COUNT * sizeof( uint32_t ), 0 );
    vkCmdFillBuffer( cmd, dispatchCmdBuffer, 0,
                     sizeof( VkDispatchIndirectCommand ), 0 );

    // zero the gradients buffer: NrcBackward only atomicAdds into it and
    // NrcOptimize never clears it, so without this fill the gradients would
    // accumulate across frames (upstream VkNRC clears them with a
    // BufferFillPass every frame, see NNTrain.hpp)
    vkCmdFillBuffer( cmd, gradientsBuffer, 0, NRC_WEIGHT_COUNT * sizeof( float ), 0 );

    // make zeroed counters visible to the raygen and compute shaders
    // that read/write them this frame
    {
        ...
        svkCmdPipelineBarrier2KHR( cmd, &depInfo );
    }
}
